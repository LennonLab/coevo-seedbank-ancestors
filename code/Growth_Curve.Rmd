...
library(broom)
library(cowplot)
setwd("C:/Users/danschw/Box/Lab/research/Bacillus phage/coevolution/growth_curve")
```

## growth in LB and DSM (+Cm)  

```{r, message=FALSE}
input <- read_csv("20200305_spoIIE_d6.csv")
#time in hours
...
input <- input[1:(8*floor(nrow(input)/8)),]
input.avg <- input[0,]
for(i in seq(nrow(input)/8)){
  row1 <- i*8-7
  input.avg[i,] <- apply(input[row1:(row1+7),], 2,mean)
...
#add metadata
meta <- read_csv("20200305_spoIIE_d6_layout.csv")
d <-merge(d, meta)
```
...
* Wells B2 and D8 probably has some particulate matter  
* blanks of A7 and H8  
...
d%>%
  ...
  ggplot(aes(x=col, y=OD600))+
    geom_col(aes(fill=interaction(strain,colony, media)))+
    theme_cowplot()+
    facet_grid(row~.)
...
d%>%
  ...
  filter(strain!="blank")%>%
  filter(!(well%in%outlier.wells))%>%
  # mutate(strain = fct_relevel(strain, "pDR110","sigF", "sigG","SP10","Goe3")) %>%
  ggplot(aes(x=Time, y=OD600))+
...
write_tsv(cbind (Min,blank,export), path = "GrowthRate/LB.txt")
...
write_tsv(cbind (Min,blank,export), path = "GrowthRate/DSM.txt")
...
lb.gr <- read_tsv("GrowthRate/LB.summary", skip=1)
dsm.gr <- read_tsv("GrowthRate/DSM.summary", skip=1)
...
gr %>%
  ...
  ggplot(aes(x=colony, y=Rate))+
  geom_jitter( aes(color=strain, shape=colony),size=3, height=0, width=0.3, fill="white")+
  theme_cowplot()+panel_border()+
  # coord_flip()+
  scale_shape_manual(values = c(21,22,24))+
  facet_grid(media~strain, scales = "free_y")+
...
gr %>%
  filter(strain!="blank")%>%
  filter(!(well%in%outlier.wells))%>%
  ggplot(aes(x=colony, y=`lag time (minutes)`))+
  geom_jitter( aes(color=strain, shape=colony),size=3, height=0, width=0.2, fill="white")+
  theme_cowplot()+panel_border()+
  # coord_flip()+
  scale_shape_manual(values = c(21,22,24))+
  facet_grid(media~strain, scales = "free_y")+
...
gr%>%
  ...
  geom_boxplot()+
  geom_jitter(aes(color=colony),shape=21, fill="white", size=2, width = 0.2)+
  theme_cowplot()+
...
gr%>%
  filter(strain!="blank")%>%
    filter(!(well%in%outlier.wells))%>%
  ggplot(aes(x=strain, y=as.numeric(`Max OD`)))+
  geom_boxplot()+
  geom_jitter(aes(color=colony),shape=21, fill="white", size=2, width = 0.2)+
  theme_cowplot()+
  facet_wrap(~media, scales = "free_y")+
...
## DSM
The plan for the co-evolution experiment is to use DSM. Significant strain dependent differences are seen for the **Max OD**. WT has a higher average Max OD, but the difference seems quite small. I should sat that seeing a difference in this parameter is not surprising, as that is the point where the two strains are expected to behave differently: WT sporulates, while $\Delta$spoIIE cannot.
...
I was thinking I would do a direct competition over several seial transfers. However that seems like a bad idea beacuse spores aould obviously have a longer lag time. So I should probably start them of from cultutres with-out sporulation, maybe repress with glucose.
...
plot_grid(p.rate,p.od,p.lag, ncol = 1)
```
