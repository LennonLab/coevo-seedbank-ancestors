...
#plot
d.plot%>%
  ggplot(aes(x=strain, y=val))+
  geom_jitter( shape=21,size=1.5, fill="grey", alpha=0.5,
               height=0, width=0.2)+
  geom_errorbar(data = d.plot.sum, aes(y=m, ymin = m-sem, ymax = m+sem),
                size=1, width=0.3)+
  ...
  scale_shape_manual(values = c(21,22,24))+
  facet_wrap(~fct_rev(param), scales = "free" )+
    theme(strip.background = element_blank(),
          strip.text = element_text(face = "bold"),
          axis.title.y = element_blank())+
  ylim(0,NA)+
  ggsave(here("plots","DSM_grwoth-params.png"), width = 7, height = 4)
